...
lemma {:axiom} delOne(m : multiset<Solution>,s:Solution)
  ...
  ensures forall e | e in Values(m) && e != s.value ::  Values(m-multiset{s})[e] ==  Values(m)[e]
  ensures s.value !in Values(m).Keys ==>  Values(m-multiset{s}).Keys == Values(m).Keys-{s.value}
  ensures s.value in Values(m).Keys ==> Values(m-multiset{s}).Keys == Values(m).Keys
  ...
  ensures Sorted(w[..]) // el nuevo array esta ordenado
{
  ...
  assert i == v.Length && w[..i] == w[..];
  assert forall e | e in multiset(v[..]) :: e !in Values(pq.Model()) && e in  multiset(w[..i]) && multiset(v[..])[e] == multiset(w[..i])[e];
